  ...
  <div class="info-form">
    <el-form :model="ruleForm"
      ...
      <el-form-item class="buttons">
        <el-button type="primary" @click="submitForm('ruleForm')" icon="el-icon-search">
          查询
        </el-button>
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <div v-html="cutString(scope.row.content)" :title="scope.row.content"></div>
        </template>
      </el-table-column>
      <el-table-column
        prop="beginsTime"
    ...
    <el-dialog width='500px' custom-class="custom-dialog"
      ...
      <div slot='footer' class='dialog-footer'>
        ...
        <el-button type='primary' @click='updateStatus' :disabled="isSubmiting">提 交</el-button>
      </div>
    </el-dialog>
    <!-- 修改信息内容对话框 -->
...
export default {
  ...
  methods: {
    ...
    timeFilter(str) {
      ...
      const m = this.fillZero((date.getMonth() + 1), 2)
      const d = this.fillZero(date.getDate(), 2)
      const h = this.fillZero(date.getHours(), 2)
      const i = this.fillZero(date.getMinutes(), 2)
    ...
    handleEndTime() {
      try {
        if (+this.ruleForm.date2 - +this.ruleForm.date1 > 8.64e7) {
    ...
    changePickerEnd() {
      ...
      this.datePickerEnd = {
        disabledDate: (time) => {
          ...
          return +time < +new Date(a)
        }
      }
    },
    changeEndTimeRange() {
      let selectableRange = ''
      const endTime = new Date(+this.ruleForm.time1 + 1000 * 60 * 60 * 6)
      const hour = this.fillZero(endTime.getHours(), 2)
      const min = this.fillZero(endTime.getMinutes(), 2)
      selectableRange = `${hour}:${min}:00 - 23:59:59`
